...
function showAddPlace() {
        ...
        <div class="place-form">
            ...
            <form id="addPlaceForm">
                ...
                <div class="form-group">
                    ...
                    <input type="time" id="placeOpenHours" value="08:00" required>
                </div>
                <div class="form-group">
                    <label>Jam Tutup</label>
                    <input type="time" id="placeCloseHours" value="22:00" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <small>Pilih gambar untuk tempat anda! (JPG, PNG, max 2MB)</small>
  ...
  document
    ...
    .addEventListener("submit", handleAddPlace);

  // Handle image preview
  document
    .getElementById("placeImageFile")
...
async function showMyPlaces() {
  const ownerContent = document.getElementById("ownerPlaces");
  ownerContent.innerHTML =
    '<div class="loading">🔄 Memuat tempat Anda...</div>';
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (ownerPlaces.length === 0) {
        ownerContent.innerHTML =
          '<div class="no-data">Belum ada tempat yang ditambahkan</div>';
      } else {
        ownerContent.innerHTML = ownerPlaces
          .map((place) => {
            const imageUrl =
              place.image ||
              "https://via.placeholder.com/300x200?text=Tempat+Nongkrong";
            return `
                        <div class="place-card" style="border-left: 4px solid ${
              ...
              place.name
            }" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='https://via.placeholder.com/300x200?text=Tempat+Nongkrong'">
                            </div>
                            <div class="place-content">
                                <h3>${place.name} <span style="color:${
              place.status === "approved"
                ? "#28a745"
                : place.status === "pending"
                ? "#ffc107"
                : "#dc3545"
            }; font-size:0.8em;">[${place.status.toUpperCase()}]</span></h3>
...
async function showBookings() {
  ...
  try {
    ...
    if (ownerBookings.length === 0) {
      ownerContent.innerHTML =
        '<div class="no-data">Belum ada booking di tempat Anda</div>';
      return;
    }
    ...
    ownerContent.innerHTML =
      '<div class="no-data">❌ Gagal memuat booking</div>';
  }
}
...
async function handleAddPlace(event) {
  ...
  try {
    ...
    const placeData = {
      ...
      openHours: document.getElementById("placeOpenHours").value,
      closeHours: document.getElementById("placeCloseHours").value,
    };
    ...
    if (response.ok) {
      ...
      showNotification(
        "❌ " +
          (result.message || `Error ${response.status}: Gagal menambah tempat`),
        "error"
      );
    }
  } catch (error) {
    ...
    if (result.found) {
      previewDiv.innerHTML = `
                <div class="location-found">
                    ...
                    🌍 Koordinat: ${result.coordinates.lat.toFixed(
                      6
    ...
    setTimeout(() => {
      if (!ownerMap) {
        ...
        ownerMap.initOwnerMap("ownerMapContainer", (lat, lng) => {
          ...
          document.getElementById("placeLocation").value = `${lat.toFixed(
            6
          )}, ${lng.toFixed(6)}`;
          ...
          document.getElementById("locationPreview").innerHTML = `
                        <div class="location-found">
                            ...
                            📍 Koordinat: ${lat.toFixed(6)}, ${lng.toFixed(
            6
          )}<br>
...
async function updateBookingStatus(bookingId, status) {
  ...
  try {
    ...
    if (result.message) {
      ...
      console.error("❌ API Error:", result);
      showNotification(
        "❌ " + (result.message || "Gagal update status booking"),
        "error"
      );
    }
  } catch (error) {
